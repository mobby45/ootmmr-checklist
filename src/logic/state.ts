...
export function buildLogicState(
  ...
  // itemMapping: bow → OOT_BOW, mm_bow → MM_BOW, shared_bow → SHARED_BOW.
  // Seed per-game: OoT bow → ARROWS; MM bow → MM_ARROWS; shared → both.
  if ((items.get('OOT_BOW') ?? 0) + (items.get('SHARED_BOW') ?? 0) > 0) events.add('ARROWS');
  if ((items.get('MM_BOW') ?? 0) + (items.get('SHARED_BOW') ?? 0) > 0) events.add('MM_ARROWS');
  // itemMapping: bomb → OOT_BOMB_BAG, mm_bomb → MM_BOMB_BAG, shared_bomb → SHARED_BOMB_BAG.
  // Seed per-game: OoT bag → BOMBS; MM bag → MM_BOMBS; shared → both.
  if ((items.get('OOT_BOMB_BAG') ?? 0) + (items.get('SHARED_BOMB_BAG') ?? 0) > 0) {
    events.add('BOMBS'); events.add('BOMBS_OR_BOMBCHU');
  }
  if ((items.get('MM_BOMB_BAG') ?? 0) + (items.get('SHARED_BOMB_BAG') ?? 0) > 0) {
    events.add('MM_BOMBS'); events.add('MM_BOMBS_OR_BOMBCHU');
  }
  // itemMapping: bombchu → OOT_BOMBCHU + license; mm_bombchu → MM_BOMBCHU + license; shared_bombchu → BOMBCHU + license.
  // Seed per-game: OoT bombchu → BOMBCHU; MM bombchu → MM_BOMBCHU; shared → both.
  if ((items.get('OOT_BOMBCHU') ?? 0) + (items.get('BOMBCHU') ?? 0) > 0) {
    events.add('BOMBCHU'); events.add('BOMBS_OR_BOMBCHU');
  }
  if ((items.get('MM_BOMBCHU') ?? 0) + (items.get('BOMBCHU') ?? 0) > 0) {
    events.add('MM_BOMBCHU'); events.add('MM_BOMBS_OR_BOMBCHU');
  }
